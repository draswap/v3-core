...
import '../../../../../contracts/DraswapV3Pool.sol';
...
contract E2E_mint_burn {
    ...
    DraswapMinter minter;
    DraswapSwapper swapper;
    ...
    constructor() public {
        ...
        draswap = new SetupDraswap(token0, token1);

        minter = new DraswapMinter(token0, token1);
        swapper = new DraswapSwapper(token0, token1);
    ...
    function check_mint_invariants(
        int24 _tickLower,
        int24 _tickUpper,
        DraswapMinter.MinterStats memory bfre,
        DraswapMinter.MinterStats memory aftr
    ) internal {
        (, int24 currentTick, , , , , ) = pool.slot0();
    ...
    function _init(uint128 _seed) internal {
        ...
        // deploy the pool
        //
        draswap.createPool(poolParams.fee, poolParams.startPrice);
        pool = draswap.pool();
    ...
    function test_mint(uint128 _amount) public {
        ...
        (DraswapMinter.MinterStats memory bfre, DraswapMinter.MinterStats memory aftr) = minter.doMint(
            _tL,
            _tU,
            _amount
        );
    ...
    function test_burn_partial(uint128 _amount) public {
        ...
        try minter.doBurn(pos.tickLower, pos.tickUpper, burnAmount) returns (
            DraswapMinter.MinterStats memory bfre_burn,
            DraswapMinter.MinterStats memory aftr_burn
        ) {
            bfre = bfre_burn;
            aftr = aftr_burn;
        } catch {
